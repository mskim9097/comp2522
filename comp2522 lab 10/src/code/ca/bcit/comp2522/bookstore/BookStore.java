...
public class BookStore
{
    ...
    public void printGroupByDecade(final int decade)
    {
        ...
        filteredStream(bookList)
                .filter(novel -> novel.getYearPublished() >= startOfDecade &&
                                 novel.getYearPublished() <= endOfDecade)
                .map(Novel::getTitle)
                .forEach(System.out::println);
    }
    ...
    {
        filteredStream(bookList)
                .max(Comparator.comparingInt(novel -> novel
                        .getTitle()
                        .length()))
                .map(Novel::getTitle)
    ...
    {
        return filteredStream(bookList)
    ...
    public int howManyBooksContain(final String word)
    {
        ...
        return (int) filteredStream(bookList)
                .map(Novel::getTitle)
                                          ...
                                          int last)
    {
        ...
        long count = filteredStream(bookList)
                .filter(novel -> novel.getYearPublished() >= from &&
                                 novel.getYearPublished() <= to)
                .count();

        return (int) (count * MAXIMUM_PERCENTAGE) / bookList.size();
    }
    ...
    /**
     * A method that filters a list of novels and returns a stream of valid novels.
     * @param bookList the list of novels to filter
     * @return a {@code Stream<Novel>} containing only valid novels
     */
    private static Stream<Novel> filteredStream(final List<Novel> bookList)
    {
        return bookList.stream()
                .filter(novel -> novel != null &&
                        novel.getTitle() != null &&
                        !novel.getTitle().isBlank() &&
                        novel.getYearPublished() >= MINIMUM_YEAR &&
